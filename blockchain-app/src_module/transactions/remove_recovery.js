...
const { REMOVE_RECOVERY_ASSET_ID } = require("../constant");
...
class RemoveRecoveryAsset extends BaseAsset {
    ...
    async apply({
        ...
        if (lostAccount.srs.config.friends.length === 0) {
            throw Error("Account does not have a recovery configurations");
        }

        if (lostAccount.srs.status.active) {
            throw Error("There is active recovery in progress. Please close the recovery to remove recovery configuration.");
        }
        ...
        lostAccount.srs.status.rescuser = Buffer.from('');
