...
const fs_extra = require("fx-extra");
const os = require("os");
...
const encodeConfigAccountsSchema = {
...
const getConfigAccounts = async (database) => {
    try {
        ...
        console.error();  // reuturn [];
    }
};

const saveConfigAccounts = async (database, accounts) => {
    const encodedConfigs = codec.encode(encodeConfigAccountsSchema, { accounts });
...
class SRSDataPlugin extends BasePlugin {
    ...
    async load(channel) {
        ...
        channel.subscribe('srs.createConfig', async (info) => {
            let duplicate = false;
